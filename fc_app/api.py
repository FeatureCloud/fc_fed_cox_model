...
from lifelines.utils import normalize, StepSizer, ConvergenceError, inv_normal_cdf, _get_index, check_nans_or_infs
...
from .cox import global_covariates_intersection, create_figure
...
def status():
    ...
    if cur_step == "local_variance":
        ...
        if redis_get('master'):
            if redis_get('iteration') == 0:
                ...
                current_app.logger.info(
                    f'[WEB] time for initializing the statistics, model parameter, step_size and so...: ' + \
        ...
        result.to_csv("/mnt/output" + '/results.csv', sep=',')
        params_ = result['coef']
        standard_errors_ = result['se(coef)']
        fig = create_figure(params_, standard_errors_)
        fig.savefig("/mnt/output" + '/plot.png')
        redis_set('master_step', 'finished')
        redis_set('slave_step', 'finished')
        redis_set('step', 'finished')
    elif cur_step == 'finished':
        current_app.logger.info('[API] Finished.')
        ...
        if master:
            ...
            if master_step == 'master_upload':
                ...
                return jsonify({'upload_info': (
                    max_steps, precision, duration_col, event_col, covariates_json, l1_ratio, penalization,
                    intersection)})
...
def local_mean_calculation():
    ...
    if redis_get('master'):
        ...
        stop_time = time.time()
        current_app.logger.info(
            f'[WEB] time for covariates intersection calculation and sending: {stop_time - start_time}')
        redis_set('start_time', stop_time)
...
def get_efron_values(global_stat):
    ...
    for client in global_stat:
        ...
        for time in sorted(D, reverse=True):
            ...
            t = str(float(
                time))  # in D there are ints but json makes out of integer keys -> string keys and in i1 there are strings of float numbers
