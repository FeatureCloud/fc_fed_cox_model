...
from .api import redis_get, redis_set, preprocessing
...
def root():
    ...
    if step == 'start':
        ...
        if not redis_get('master'):
            ...
            if penalization == 0:
                ...
                if l1_ratio == 0:
                    ...
                    penalizer = 'elastic net penalized regression with penalizer: ' + str(
                        penalization) + ' and l1_ratio: ' + str(l1_ratio)

            return render_template('setup_client.html', max_steps=max_steps, precision=precision,
                                   duration_col=duration_col, event_col=event_col, covariates=covariates,
                                   penalizer=penalizer, intersect=intersect)
        else:
            return render_template('calculations_new.html')

    elif step == 'final':
        pd.options.display.float_format = '{:,.4f}'.format
        result = redis_get('result')
        # result = result.drop(['se(coef)','z','exp(coef) lower 95%','exp(coef) upper 95%'],axis=1)
        penalization = redis_get('penalization')
        # if a variable selection method was executed, just the covariates with p-values<=0.05 are shown
        # in the final.html site
        if penalization == 0:
            result_html = result.to_html(classes='data', header='true', justify="center")
        else:
        ...
        current_app.logger.info(f'[API] write results into output folder')
        result.to_csv(OUTPUT_DIR + '/results.csv', sep=',')
        params_ = result['coef']
        standard_errors_ = result['se(coef)']
        fig = create_figure(params_, standard_errors_)
        fig.savefig(OUTPUT_DIR + '/plot.png')
        redis_set('master_step', 'finished')
        redis_set('slave_step', 'finished')
        return render_template('final_new.html', tables=[result_html], c_index=c_index)
    else:
        return render_template('calculations_new.html')
                    ...
                    if (duration_col in data.columns) and (event_col in data.columns):
                        ...
                        if intersection == 1:
                            ...
                            if not intersect_covariates.empty:
                                ...
                                # the intersection of master and client is null
                                # show again setup page with error message
                            ...
                            if set(master_covariates) == set(local_covariates):
                                ...
                                current_app.logger.info(
                                    '[WEB] No intersection provided, covariates differ from those of master')
                                error_message = 'All given covariates must be in the dataset. ' + \
                                                'Intersection not provided, please do setup again!'
                                flash(error_message)
                                return redirect(url_for('web.root'))
